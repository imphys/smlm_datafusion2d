%one2all_symmetric   Register each particle to the stack of all the remaining
% particles and iterate, exactly the same as one2all() but with taking into
% account the prior knowledge about the symmetry of NPC data
%
%   SYNOPSIS:
%       [ superParticle, parameter ] = one2all_symmetric(Particles, iter)
%
%   Input: 
%       Particles: Cell array of particles of size 1xN
%       iter: the number of iterations
%       outdir: output directory to store final super-particle
%       oldM: the 3x3xN matrix containig transformation parameters after
%       step 2
%       outdir: output direcotry where the results are stored
%
%   Output:
...
%             M(:,:,i,j) = M(:,:,i,j) * oldM(:,:,i);
    ...
    % save to disk
    save([outdir '/superParticleSymm'], 'superParticle');
