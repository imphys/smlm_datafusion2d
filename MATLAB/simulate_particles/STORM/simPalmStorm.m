...
function [coords, rigidParam, phantom, binding_sites, siteID] = simPalmStorm( CCD_pixelsize, box_width, zoom, ...
   timeframes, bg, n_photons, t_on, t_off, ...
...
% idT = find(emitter_locs(:,3)>0);
% idB = find(emitter_locs(:,3)<0);
% rand_height = 8*(2*rand-1)/CCD_pixelsize;
% emitter_locsT = emitter_locs(idT,:)+[0 0 rand_height];
% emitter_locs = [emitter_locsT;emitter_locs(idB,:)];
%%
...
%     particle_size = size(emitter_locs, 1);
    ...
    % random rotation
%         angle = 2*pi*rand(1,3);             % [yaw, pitch, roll]
%     angle = (2*rand(1,3)-1)*pi;             % [yaw, pitch, roll]
%         qtmp = angle2quat(angle(1), angle(2)/36/2, angle(3)/36/2);      % constraint yaw/pitch rotations % 1st try 5 deg
%     qtmp = angle2quat(angle(1), angle(2)/9/1, angle(3)/9/1);      % constraint yaw/pitch rotations % 2nd try +/-20 deg
...
%     end
% end
[emitter_locs_on, t, siteID] = simlocs(emitter_locs, 1, switching_params, localization_params);
% Q = mean(M);
